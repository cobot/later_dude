...
class CalendarTest < ActiveSupport::TestCase
  ...
  test "uses full day names" do
    assert_equal FULL_DAY_NAMES, LaterDude::Calendar.new(2009, 1, :use_full_day_names => true).renderer.send(:day_names)
  end

  test "uses abbreviated day names" do
    assert_equal ABBR_DAY_NAMES, LaterDude::Calendar.new(2009, 1).renderer.send(:day_names)
    assert_equal ABBR_DAY_NAMES, LaterDude::Calendar.new(2009, 1, :use_full_day_names => false).renderer.send(:day_names)
  end

  test "doesn't show day names" do
    assert_nil LaterDude::Calendar.new(2009, 1, :hide_day_names => true).renderer.send(:show_day_names)
  end

  # TODO I think this isn't needed anymore due to the new :header_date_format option
  test "doesn't show month names" do
    assert_nil LaterDude::Calendar.new(2009, 1, :hide_month_name => true).renderer.send(:show_month_names)
  end

  # links/texts for next, previous and current month
  test "uses next month for a string" do
    assert_match %r(&raquo;), LaterDude::Calendar.new(2009, 1, :next_month => "&raquo;").renderer.send(:next_month)
  end

  test "uses next month for a strftime string" do
    assert_match %r(Feb), LaterDude::Calendar.new(2009, 1, :next_month => "%b").renderer.send(:next_month)
  end

  test "uses next month for proc" do
    assert_match %r(<a href="/events/2009/2">&raquo;</a>), LaterDude::Calendar.new(2009, 1, :next_month => lambda { |date| %(<a href="/events/#{date.year}/#{date.month}">&raquo;</a>) }).renderer.send(:next_month)
  end

  test "uses next month for array if first value is a string and second is a proc" do
    assert_match %r(<a href="/events/2009/2">&raquo;</a>), LaterDude::Calendar.new(2009, 1, :next_month => ["&raquo;", lambda { |date| "/events/#{date.year}/#{date.month}" }]).renderer.send(:next_month)
  end

  test "uses next month for array if first value is a strftime string and second is a proc" do
    assert_match %r(<a href="/events/2009/2">Feb</a>), LaterDude::Calendar.new(2009, 1, :next_month => ["%b", lambda { |date| "/events/#{date.year}/#{date.month}" }]).renderer.send(:next_month)
  end

  test "uses previous month for a string" do
    assert_match %r(&laquo;), LaterDude::Calendar.new(2009, 1, :previous_month => "&laquo;").renderer.send(:previous_month)
  end

  test "uses previous month for a strftime string" do
    assert_match %r(Dec), LaterDude::Calendar.new(2009, 1, :previous_month => "%b").renderer.send(:previous_month)
  end

  test "uses previous month for proc" do
    assert_match %r(<a href="/events/2008/12">&laquo;</a>), LaterDude::Calendar.new(2009, 1, :previous_month => lambda { |date| %(<a href="/events/#{date.year}/#{date.month}">&laquo;</a>) }).renderer.send(:previous_month)
  end

  test "uses previous month for array if first value is a string and second is a proc" do
    assert_match %r(<a href="/events/2008/12">&laquo;</a>), LaterDude::Calendar.new(2009, 1, :previous_month => ["&laquo;", lambda { |date| "/events/#{date.year}/#{date.month}" }]).renderer.send(:previous_month)
  end

  test "uses previous month for array if first value is a strftime string and second is a proc" do
    assert_match %r(<a href="/events/2008/12">Dec</a>), LaterDude::Calendar.new(2009, 1, :previous_month => ["%b", lambda { |date| "/events/#{date.year}/#{date.month}" }]).renderer.send(:previous_month)
  end
  ...
  test "uses next and previous month for array if first value is a strftime string and second is a proc" do
    ...
    assert_match %r(<a href="/events/2008/12">Dec</a>), calendar.renderer.send(:previous_month)
    assert_match %r(<a href="/events/2009/2">Feb</a>), calendar.renderer.send(:next_month)
  end
  ...
  test "uses next/previous month options rather than combined option if both are given" do
    ...
    assert_match %r(&laquo;), calendar.renderer.send(:previous_month)
    assert_match %r(&raquo;), calendar.renderer.send(:next_month)
  end

  test "uses current month for a string" do
    assert_match %r(Current), LaterDude::Calendar.new(2009, 1, :current_month => "Current").renderer.send(:current_month)
  end

  test "uses current month for a strftime string" do
    assert_match %r(Jan), LaterDude::Calendar.new(2009, 1, :current_month => "%b").renderer.send(:current_month)
  end

  test "uses current month for proc" do
    assert_match %r(<a href="/events/2009/1">Current</a>), LaterDude::Calendar.new(2009, 1, :current_month => lambda { |date| %(<a href="/events/#{date.year}/#{date.month}">Current</a>) }).renderer.send(:current_month)
  end

  test "uses current month for array if first value is a string and second is a proc" do
    assert_match %r(<a href="/events/2009/1">Current</a>), LaterDude::Calendar.new(2009, 1, :current_month => ["Current", lambda { |date| "/events/#{date.year}/#{date.month}" }]).renderer.send(:current_month)
  end

  test "uses current month for array if first value is a strftime string and second is a proc" do
    assert_match %r(<a href="/events/2009/1">Jan</a>), LaterDude::Calendar.new(2009, 1, :current_month => ["%b", lambda { |date| "/events/#{date.year}/#{date.month}" }]).renderer.send(:current_month)
  end
  ...
  test "includes day name abbreviation" do
    assert_equal %q(<abbr title="Sunday">Sun</abbr>), LaterDude::Calendar.new(2009, 1, :use_full_day_names => false).renderer.send(:include_day_abbreviation, "Sun")
  end

  test "doesn't include day name abbreviation" do
    assert_equal "Sunday", LaterDude::Calendar.new(2009, 1, :use_full_day_names => true).renderer.send(:include_day_abbreviation, "Sunday")
  end

  test "shows index of first and last day of week" do
    assert_equal 0, LaterDude::Calendar.new(2009, 1).renderer.send(:first_day_of_week)
    assert_equal 6, LaterDude::Calendar.new(2009, 1).renderer.send(:last_day_of_week)

    assert_equal 1, LaterDude::Calendar.new(2009, 1, :first_day_of_week => 1).renderer.send(:first_day_of_week)
    assert_equal 0, LaterDude::Calendar.new(2009, 1, :first_day_of_week => 1).renderer.send(:last_day_of_week)
  end

  test "applies first day of week accordingly" do
    assert_equal %w(Mon Tue Wed Thu Fri Sat Sun), LaterDude::Calendar.new(2009, 1, :first_day_of_week => 1).renderer.send(:apply_first_day_of_week, ABBR_DAY_NAMES)
  end

  test "returns the first day of the week for a given date" do
    day = Date.civil(2009, 1, 4) # first Sunday in 2009

    assert_equal Date.civil(2009, 1, 4),   LaterDude::Calendar.new(2009, 1, :first_day_of_week => 0).renderer.send(:beginning_of_week, day)
    assert_equal Date.civil(2008, 12, 29), LaterDude::Calendar.new(2009, 1, :first_day_of_week => 1).renderer.send(:beginning_of_week, day)
    assert_equal Date.civil(2008, 12, 30), LaterDude::Calendar.new(2009, 1, :first_day_of_week => 2).renderer.send(:beginning_of_week, day)
    assert_equal Date.civil(2008, 12, 31), LaterDude::Calendar.new(2009, 1, :first_day_of_week => 3).renderer.send(:beginning_of_week, day)
    assert_equal Date.civil(2009, 1, 1),   LaterDude::Calendar.new(2009, 1, :first_day_of_week => 4).renderer.send(:beginning_of_week, day)
    assert_equal Date.civil(2009, 1, 2),   LaterDude::Calendar.new(2009, 1, :first_day_of_week => 5).renderer.send(:beginning_of_week, day)
    assert_equal Date.civil(2009, 1, 3),   LaterDude::Calendar.new(2009, 1, :first_day_of_week => 6).renderer.send(:beginning_of_week, day)
  end
  ...
  test "shows previous month if first day of month isn't the first day of a week" do
    assert_not_nil LaterDude::Calendar.new(2009, 4).renderer.send(:show_previous_month) # April 1st is a Wednesday
  end

  test "shows following month if last day of month isn't the last day of a week" do
    assert_not_nil LaterDude::Calendar.new(2009, 4).renderer.send(:show_following_month) # April 30th is a Thursday
  end

  test "shouldn't show previous month if first day of month is the first day of a week" do
    assert_nil LaterDude::Calendar.new(2009, 2).renderer.send(:show_previous_month) # February 1st is a Sunday
  end

  test "shouldn't show following month if last day of month is the last day of a week" do
    assert_nil LaterDude::Calendar.new(2009, 2).renderer.send(:show_following_month) # February 28th is a Saturday
  end
  ...
  test "first_rendered_date returns first date for current month that calendar render" do
